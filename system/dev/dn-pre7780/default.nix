...
}:
{
  imports = [
    ...
    #   https = false;
    # })
    (import ../../modules/mail-server.nix {
      fqdn = "vmail.net.dn";
      networks = [
        "127.0.0.0/8"
        "10.0.0.0/24"
      ...
      enableOpenldap = true;
      dovecotLdapSecretFile = config.sops.secrets."dovecot/openldap".path;
      openldapAdmPassPath = config.sops.secrets."openldap/adminPassword".path;
      sslCertPath = "/etc/letsencrypt/live/vmail.net.dn/fullchain.pem";
      sslKeyPath = "/etc/letsencrypt/live/vmail.net.dn/privkey.pem";
    })
  ];
