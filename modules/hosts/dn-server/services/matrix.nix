...
let
  ...
  inherit (config.flake.public.config.services.matrix)
    endpoint
    hostname
    web
    ;
  ...
  clientConfig = {
    ...
    "org.matrix.msc4143.rtc_foci" = [
      {
        type = "livekit";
        livekit_service_url = "${endpoint}/livekit/jwt";
      }
    ];
  };
  serverConfig = {
  ...
  configurations.nixos.dn-server.module =
    { pkgs, ... }:
    {
      ...
      services.matrix-synapse = {
        ...
        settings = {
          server_name = domain;
          web_client_location = web.endpoint;
          public_baseurl = endpoint;
      ...
      services.lk-jwt-service = {
        enable = true;
        port = 8198;
        livekitUrl = "wss://${hostname}/livekit/sfu";
        keyFile = livekitKeyFile;
      };
      ...
      systemd.services.lk-jwt-service.environment.LIVEKIT_FULL_ACCESS_HOMESERVERS = "${hostname}";
